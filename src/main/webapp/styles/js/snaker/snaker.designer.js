(function(snaker){
    ...
    designer.util = {
        ...
        tip : function(rect, name) {
            ...
            tipDIV.style.width=380;
            tipDIV.style.height=60;
            tipDIV.style.position="absolute";
            tipDIV.style.backgroundColor="#FFE699";
            $.ajax({
                type:'GET',
                url:designer.config.ctxPath+"/Platform/workflow/webFindTipsJson",
                data:"businessId=" + designer.config.businessId + "&taskName=" + name,//_o.props['name'].value,
                dataType:'json',
                async: false,
                error: function(){
                    alert('数据处理错误！');
                ...
                success: function(result){
                	...
                	tipDIV.innerHTML="<div style='width:380px; height:20px;border: 1px solid #d2dde2;'><a href='javascript:void(0)' onclick='document.body.removeChild(document.getElementById(\"tipDIV\"))' class='btnDel'></a></div><div style='width:380px; height:40px;border: 1px solid #d2dde2;'><div id='currentActorDIV' style='width:380px; height:20px;'>参与者:" + data.actors + "</div><div id='arrivalDIV' style='width:380px; height:20px;'>抵达时间:" + data.createTime + "</div></div>";
